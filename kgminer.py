...
def write_to_kgminer(poi, q_part, resource_v, sentence_id, data_source, kr):
    ...
    if kr:
        ...
        word_vec_train_zip = zip(*[iter(word_vec_train)] * 2)
        word_vec_train_zip = [list(elem) for elem in word_vec_train_zip]
        csv_writer(word_vec_train_zip, data_source + '/' + sentence_id + data_source + '_random')
        if len(word_vec_train) > 10:
            node_ids = entity_id_finder(word_vec_train)
            training_data, test_data = train_data_csv(word_vec_train, node_ids, resource_v)
            csv_writer(training_data, file_name=data_source + '/' + sentence_id + data_source + 'random_ids')
            copyfile(KGMiner_data + '/' + data_source + '/' + sentence_id + data_source + 'random_ids.csv', \
                     KGMiner_data + '/' + 'training_data.csv')
            return True
        else:
            ...
            if len(triple_v) <= len(resource_ids):
                ...
                for predicate_of_interest in predicate_set[0][:10]:
                    if None not in test_data and predicate_of_interest and kgminer_status is False:
                        ...
                        if kr:
                            if sentence_id+data_source+'_random_ids.csv' not in training_files:
                                kgminer_status = write_to_kgminer(poi, q_part, resource_v, sentence_id, data_source, kr)
                            else:
                                kgminer_status = True
                                copyfile(KGMiner_data+'/'+data_source+'/'+sentence_id+data_source+'_random_ids.csv', \
                                         KGMiner_data+'/' + 'training_data.csv')
                                copyfile(KGMiner_data + '/' + data_source + '/' + sentence_id + data_source + '_poi.csv',
                                         KGMiner_data + '/' + 'poi.csv')
                        else:
...
def word2vec_dbpedia(train_ents, resource_v):
    ...
    for j in range(0, len(train_ents) - 1, 2):
        if entity_tracker.get(train_ents[j+1], 0) < 3 and entity_tracker.get(train_ents[j], 0) < 3:
            try:
