...
def possible_predicate_type(type_set, ex_ent_all):
    predicate_list = []
    count = 0
    sort_list = {}
    # for triples_v in ex_ent_all:
    # for triple_v in ex_ent_all:
    item1_v = type_set.get(ex_ent_all[0],[])
    item2_v = type_set.get(ex_ent_all[1],[])
    for it1 in item1_v:
        for it2 in item2_v:
            if it1 != it2:
...
def get_training_set(predicate_ranked, resource_type_set_ranked, ontology_threshold_ranked, ex_ent_all):
    ...
    q_part, q_part_res = or_query_prep(resource_type_set_ranked,ontology_threshold_ranked, ex_ent_all)
    ...
    for sent_pred in predicate_ranked.keys():
        ...
        for poi in predicate_of_interest:
            ...
            q_ts = 'PREFIX dbo: <http://dbpedia.org/ontology/> select distinct ?url1 ?url2 where { {?url1 <http://dbpedia.org/ontology/' + poi[
                0] + '> ?url2} . ' + q_part+q_part_res+' FILTER(?url1 != ?url2).} '
            print q_ts
            # sys.exit(0)
            try:
            ...
            if training_set:
                ...
                for j in range(0, len(train_ents)-1, 2):
                    ...
                    try:
                        ...
                        sim1 = global_settings.model_wv.similarity('DBPEDIA_ID/' + ex_ent_all[0],
                                                                  'DBPEDIA_ID/' + train_ents[j])
                        # print sim1
                        sim2 = global_settings.model_wv.similarity('DBPEDIA_ID/' + ex_ent_all[1],
